...
最初迁移时正文清理前后文本一致，核验结果见 content-extraction-report.json。后续按作者要求进行了署名匿名化，见本文件末尾的编辑记录。未将抓取用的原始网页、账号信息或私人资料放入站点资源。

## 档案

学校、岗位、奖项和历史成绩来自用户提供的简历与本次确认；身份展示按本人要求统一使用昵称「徒手拆机甲」。教育年份采用用户确认的本科 2019–2023、硕士 2025 年至今。2026-09-08 用户确认西北工业大学硕士专业为软件工程，新增华为杯中国研究生数学建模竞赛全国三等奖（硕士一年级）和京东编程与算法设计大赛全国百强。华为杯不推测具体年份与届次。用户随后确认京东百强为 2022 年本科阶段成绩，未提供月份。原有四项本科奖项均来自简历；按用户要求不展示蓝桥杯省一、国三。Codeforces 1951 / Candidate Master 继续明确标为历史成绩。全部奖项使用平行的紧凑记录，不单独放大华为杯。
...
按本人要求，README、开屏、工作台、个人档案、页面标题和描述统一改用昵称「徒手拆机甲」。旧文 `cnblogs-14620094` 的队员署名及对应标题锚点、`cnblogs-14136622` 的出题署名也替换为昵称；正文中的其他作者、算法内容、日期、文章 ID、slug 和原文链接保持原记录。

此前的迁移审计文件记录导入时的源数据，不代表本次编辑后的逐字散列。博客园原站和被忽略的本机原始备份仍是原始来源，本次没有修改它们。后续同步须保留这项匿名化编辑。
