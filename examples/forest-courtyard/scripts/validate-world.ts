...
const catalog = JSON.parse(
  await readFile('lib/content/catalog.json', 'utf8'),
);
const articleIds = new Set<string>();
const articleSlugs = new Set<string>();
for (const article of catalog.posts) {
  assert(typeof article.id === 'string' && article.id.length > 0, '文章缺少 ID');
  assert(!articleIds.has(article.id), `重复文章 ID: ${article.id}`);
  assert(
    typeof article.slug === 'string' && /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(article.slug),
    `文章地址无效: ${article.id}`,
  );
  assert(!articleSlugs.has(article.slug), `重复文章地址: ${article.slug}`);
  assert(['undergraduate', 'graduate', 'life'].includes(article.chapter), `未知栏目: ${article.id}`);
  assert.equal(article.status, 'published', '学习案例只包含已确认的教学内容');
