...
export default function WorldShell({
  ...
  const openArticle = useCallback((id: string) => {
    ...
    contentSource
      .get(id)
      .then((document) => {
        if (!document) throw new Error('这篇示例文章暂时无法打开。');
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem('forest-courtyard-example-entered'))
        setWelcome(false);
    } catch {}
    return () => {
  ...
  function start(read = false) {
    try {
      sessionStorage.setItem('forest-courtyard-example-entered', '1');
    } catch {}
    if (read) {
      setWelcome(false);
      setChapter('all');
    ...
    >
      ...
      <header className="world-header">
        ...
        >
          <WalkerPortrait />
          <span>
            林间小院<small>LEARNING EXAMPLE</small>
          </span>
        </a>
        <nav aria-label="主导航">
          <Button
